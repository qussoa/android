...
/*
    안드로이드 이벤트 핸들로 2번째로 많이 사용하는 방법
    Activity 자체를 이벤트 핸들러로 선언하기
    View.on** 클래스를 implement 해주고
    on**() method를 구현해주는 방식

    사용할때는 view.setOn**(this) 키워드 방식
 */
...
//
//
//        btn1.setOnClickListener(new myViewClass());
    ...
    /*
        ...
        이벤트를 처리해야 할 view(comp)를 대체로 이벤트 처리 코드가 유사한 경우
        한번만 이벤트 처리 코드를 작성하고 view들이 공유해서 사용하기 위함이다

        v.getId() method를 사용해서 누가 호출했는가를 구분해서 코드를 진행하면된다
     */
